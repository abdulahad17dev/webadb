...
function requiredFeaturesSupported() {
  switch (
    window.location.protocol // To work, IndexedDB pages must be served via the http or https protocol (or, for apps in the new Windows UI, the ms-wwa or ms-wwa-web protocols).
  ) {
    case "http:":
      break;
    case "https:":
    ...
    default:
      document.getElementById("bodyElement").innerHTML =
        "<h3>IndexedDB pages must be served via the http:// or https:// protocol - resolve this issue and try again.</h3>";
      return false;
  } // switch

  //   if (!document.getElementById("fileSelector").files) {
  //     document.getElementById("bodyElement").innerHTML =
  //       "<h3>File API is not fully supported - upgrade your browser to the latest version.</h3>";
  //     return false;
  //   }
  ...
  if (!window.indexedDB) {
    if (window.mozIndexedDB) {
      ...
      document.getElementById("bodyElement").innerHTML =
        "<h3>IndexedDB is not supported - upgrade your browser to the latest version.</h3>";
      return false;
    }
  } // if

  if (!window.indexedDB.deleteDatabase) {
    // Not all implementations of IndexedDB support this method, thus we test for it here.
    document.getElementById("bodyElement").innerHTML =
      "<h3>The required version of IndexedDB is not supported.</h3>";
    return false;
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (requiredFeaturesSupported()) {
    ...
    document
      ...
      .addEventListener("click", populateDB, false);
    document
      .getElementById("displayButton")
      .addEventListener("click", displayDB, false);
    document
      .getElementById("deleteButton")
      .addEventListener("click", deleteDB, false);
    document
    ...
    document
      ...
      .addEventListener("change", handleFileSelection, false); // Add an onchange event listener for the <input id="fileSelector"> element.
  } // if
});
...
function openDB() {
  ...
  try {
    ...
    openRequest.onerror = function (evt) {
      console.log(
        "openRequest.onerror fired in openDB() - error: " +
...
function handleFileSelection(evt) {
  ...
  if (!files) {
    displayMessage(
      "<p>At least one selected file is invalid - do not select any folders.</p><p>Please reselect and try again.</p>"
    );
    return;
  }
  ...
  transaction.onerror = function (evt) {
    console.log(
      "transaction.onerror fired in handleFileSelection() - error code: " +
        (evt.target.error ? evt.target.error : evt.target.errorCode)
    );
  };
  ...
  try {
    ...
    for (var i = 0, file; (file = files[i]); i++) {
      ...
      putRequest.onerror = function (evt) {
        console.log(
          "putRequest.onerror fired in handleFileSelection() - error code: " +
            (evt.target.error ? evt.target.error : evt.target.errorCode)
        );
      };
    } // for
    ...
    // try
    console.log(
      "Transaction and/or put() exception in handleFileSelection() - " +
        ex.message
    );
    return;
  } // catch
...
function populateDB() {
  ...
  var message =
    "<p>Using the below <strong>Browse</strong> button, select one or more files to store in the database.</p>";
  message +=
    "<p>Then, click the <strong>Display DB</strong> button to display what's currently in the database.</p>";
  displayMessage(message);
} // populateDB
  ...
  try {
    ...
    try {
      ...
      cursorRequest.onsuccess = function (evt) {
        ...
        if (dbGlobals.empty) {
          displayMessage(
            "<p>The database is empty &amp;ndash; there's nothing to display.</p>"
          );
        }
      ...
      // inner try
      console.log(
        "Inner try exception in displayDB() - " + innerException.message
      );
    } // inner catch
  } catch (outerException) {
    // outer try
    console.log(
      "Outer try exception in displayDB() - " + outerException.message
    );
  } // outer catch
} // displayDB
...
function deleteDB() {
  ...
  try {
    ...
    deleteRequest.onerror = function (evt) {
      console.log(
        "deleteRequest.onerror fired in deleteDB() - " +
          (evt.target.error ? evt.target.error : evt.target.errorCode)
      );
    };
...
function download() {
  const db = dbGlobals.db;

  const transaction = db.transaction([dbGlobals.storeName], "readonly");
  ...
  getRequest.onsuccess = function () {
    ...
    if (cursor) {
      ...
      const filetype = cursor.value.name.split(".").pop().toLowerCase();
      const fileName = `downloaded_file.${filetype}`; // You can provide a dynamic name or extension
