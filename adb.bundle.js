...
function sign(privateKey, data) {
  ...
  index += 1;
  const fillLength =
    padded.length - Sha1DigestInfo.length - data.byteLength - 1;
...
function defineSimpleAccessors(object, field) {
  Object.defineProperty(object, field, {
    ...
    enumerable: true,
    get() {
      return getBackingField(object, field);
    },
    set(value) {
      setBackingField(object, field, value);
    },
  });
}
...
registerFieldTypeDefinition(placeholder(), placeholder(), {
  ...
  serialize({ context, dataView, field, object, offset }) {
    ...
    (_a = backingField.buffer) !== null && _a !== void 0
      ? _a
      : (backingField.buffer = context.encodeUtf8(backingField.string));
...
registerFieldTypeDefinition(placeholder(), undefined, {
  ...
  async deserialize({ context, field, options }) {
    ...
    const view = new DataView(buffer);
    const value = view[Number$1.DataViewGetterMap[field.subType]](
      0,
  ...
  serialize({ dataView, field, object, offset, options }) {
    dataView[Number$1.DataViewSetterMap[field.subType]](
      offset,
      object[field.name],
      options.littleEndian
    );
...
(function (VariableLengthArray) {
  ...
  (function (EmptyBehavior) {
    ...
    (EmptyBehavior =
      VariableLengthArray.EmptyBehavior ||
      (VariableLengthArray.EmptyBehavior = {}))
  );
      ...
      case Array.SubType.ArrayBuffer:
        Object.defineProperty(object, field.name, {
          ...
          set(buffer) {
            ...
            setLengthBackingField(
              object,
              field,
              (_a =
                buffer === null || buffer === void 0
                  ? void 0
                  : buffer.byteLength) !== null && _a !== void 0
                ? _a
                : 0
            );
          },
...
registerFieldTypeDefinition(placeholder(), placeholder(), {
  ...
  async deserialize({ context, field, object }) {
    ...
    if (length === 0) {
      if (
        field.options.emptyBehavior === VariableLengthArray.EmptyBehavior.Empty
      ) {
        switch (field.subType) {
          case Array.SubType.ArrayBuffer:
...
function encodeBase64(input, arg1, arg2, _arg3, _arg4) {
  ...
  const [outputLength, paddingLength] =
    calculateBase64EncodedLength(inputLength);
  let maybeOutput = arguments[outputArgumentIndex];
  let outputType;
  if (maybeOutput) {
    outputOffset =
      (_a = arguments[outputArgumentIndex + 1]) !== null && _a !== void 0
        ? _a
        : 0;
...
class EventQueue {
  ...
  get onLowWater() {
    return this.lowWaterEvent.event;
  }
  push(value, size = 1) {
    if (this.ended) {
      return true;
...
class AdbReadableStream {
  ...
  get backend() {
    return this.stream.backend;
  }
  get localId() {
    return this.stream.localId;
  }
  get remoteId() {
...
class BufferedStream {
  ...
  close() {
    var _a, _b;
    (_b = (_a = this.stream).close) === null || _b === void 0
      ? void 0
      : _b.call(_a);
  }
}
...
class AdbStreamController extends AutoDisposable {
  ...
  get backend() {
    return this.dispatcher.backend;
  }
  get closed() {
    return this._closed;
  }
  get onClose() {
    return this.closeEvent.event;
  }
  ...
  async write(data) {
    ...
    for await (const chunk of chunkArrayLike(
      data,
      this.dispatcher.maxPayloadSize
    )) {
...
class AdbStream {
  ...
  get onData() {
    return this.controller.dataEvent.event;
  }
  get onClose() {
    return this.controller.onClose;
  }
  write(data) {
      ...
      init = {
        ...
        payload:
          typeof payload === "string"
            ? this.backend.encodeUtf8(payload)
            : payload,
      };
    }
    if (init.payload && init.payload.byteLength > this.maxPayloadSize) {
...
const AdbPacketWithoutPayload = new Struct({ littleEndian: true })
  ...
  .uint32("checksum")
  .int32("magic");
const AdbPacketStruct = AdbPacketWithoutPayload.arrayBuffer("payload", {
...
class AdbAuthenticationHandler {
  ...
  dispose() {
    var _a, _b;
    (_b =
      (_a = this.iterator) === null || _a === void 0 ? void 0 : _a.return) ===
      null || _b === void 0
      ? void 0
      : _b.call(_a);
  }
...
class AdbDemoMode extends AdbCommandBase {
  async getAllowed() {
    const result = await this.adb.exec(
      "settings",
      "get",
      "global",
      AdbDemoMode.AllowedSettingKey
    );
...
async function install(adb, apk, onProgress) {
  console.log(adb, apk, onProgress);
...
const AdbReverseErrorResponse = AdbReverseStringResponse.afterParsed(
  (value) => {
...
class AdbReverseCommand extends AutoDisposable {
  ...
  async sendRequest(service) {
    ...
    const success =
      this.dispatcher.backend.decodeUtf8(await stream.read(4)) === "OKAY";
    if (!success) {
  ...
  async add(deviceAddress, localPort, handler) {
    const stream = await this.sendRequest(
      `reverse:forward:${deviceAddress};tcp:${localPort}`
    );
  ...
  async remove(deviceAddress) {
    ...
    if (this.deviceAddressToLocalPort.has(deviceAddress)) {
      this.localPortToHandler.delete(
        this.deviceAddressToLocalPort.get(deviceAddress)
      );
...
const AdbSyncNumberRequest = new Struct({ littleEndian: true })
  .string("id", { length: 4 })
  .uint32("arg");
const AdbSyncDataRequest = AdbSyncNumberRequest.arrayBuffer("data", {
  lengthField: "arg",
});
...
const AdbSyncEntryResponse = AdbSyncLstatResponse.afterParsed()
  .uint32("nameLength")
...
const AdbSyncOkResponse = new Struct({ littleEndian: true }).uint32("unused");
...
const AdbSyncMaxPacketSize = 64 * 102400;
async function adbSyncPush(
  stream,
  filename,
  content,
  mode = (LinuxFileType.File << 12) | 0o666,
  mtime = (Date.now() / 1000) | 0,
  packetSize = AdbSyncMaxPacketSize,
  onProgress
) {
...
class AdbWebBackend {
  ...
  static isSupported() {
    var _a;
    return !!((_a = window.navigator) === null || _a === void 0
      ? void 0
      : _a.usb);
  }
      ...
      for (const interface_ of configuration.interfaces) {
        for (const alternate of interface_.alternates) {
          if (
            ...
            if (
              ((_a = this._device.configuration) === null || _a === void 0
                ? void 0
...
class Adb {
  ...
  get backend() {
    return this.packetDispatcher.backend;
  }
  get onDisconnected() {
    return this.backend.onDisconnected;
  }
  get connected() {
  ...
  async connect(authenticators = AdbDefaultAuthenticators) {
    var _a, _b;
    await ((_b = (_a = this.backend).connect) === null || _b === void 0
      ? void 0
      : _b.call(_a));
    ...
    const resolver = new PromiseResolver();
    const authHandler = new AdbAuthenticationHandler(
      authenticators,
      this.backend
    );
    const disposableList = new DisposableList();
    disposableList.add(
      this.packetDispatcher.onPacket(async (e) => {
        e.handled = true;
        const { packet } = e;
        try {
          switch (packet.command) {
            case AdbCommand.Connect:
              this.packetDispatcher.maxPayloadSize = Math.min(
                maxPayloadSize,
                packet.arg1
              );
            ...
            default:
              throw new Error(
                "Device not in correct state. Reconnect your device and try again"
              );
          }
        } catch (e) {
          resolver.reject(e);
    ...
    disposableList.add(
      this.packetDispatcher.onError((e) => {
        resolver.reject(e);
      })
    );
